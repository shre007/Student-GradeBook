...
if (isset($_SESSION['user'])){
    $id=substr($_SESSION['user'],0,10);
        ...
        if(isset($_POST['pyear'])&& isset($_POST['psemester'])){
            ...
            $semester=mysql_real_escape_string($_POST['psemester']);
            $subject=mysql_real_escape_string($_POST['psubject']);
            try{
       ...
       if(isset($_POST['newgrade'])&& isset($_POST['gradeid'])){
        ...
        //    $result=mysqli_query($conn,$sql);
           ...
           $file=fopen('logs/grades.txt', 'a');
           fwrite($file,"Professor with id: ".$professorid." has updated grade of student with id: ".$studentid." enrolled in: ".$courseid." at: ".date('Y-m-d H:i:s')."\n");
           foreach($_SERVER as $key => $value){
fwrite($file,$_SERVER[$key]."\n");
}
          fclose($file);
           if($rows==0){
               echo "4";
